...
_list*   analizador_evalua_movidas( Regla* regla, Posicion* pos, Pieza* pieza, Casillero* cas, char tipoanalisis, int tipomovida, int color ){
    ...
    posicion_copy( ZPOSICION(z), pos );
    z->pieza   = posicion_get_pieza(ZPOSICION(z),pieza);
    z->cas     = cas;
    z->cas_ori = cas;
    z->color   = color;
    ...
    if( z->movidas && z->flags & CON_TRANSFORMACION ) movida_split_transformaciones( z->movidas );
...
int    analizador_atacado( Analizador* z, Casillero* cas ){
    ...
    posicion_mueve_pieza( ZPOSICION(z), z->pieza, ccc );
    Posicion* pos = posicion_dup( ZPOSICION(z) );
    ...
    for( i = 1; i <= z->pos.tjuego->colores; i ++ ){
        ...
        while( mov = (Movida*) list_siguiente( pos->movidas ) ){
            if( movida_casillero_destino( mov ) == ccc ){
                ...
                posicion_free( pos );
                posicion_mueve_pieza( ZPOSICION(z), z->pieza, cant );
                return 1;
            }
        }
    }
...
int    analizador_juega  ( Analizador* z, Casillero* cas, int con_captura, int continua ){
    ...
    if( !z->mov_actual ) z->mov_actual = movida_new( ZPOSICION(z), z->pieza, z->tmov );
...
int   analizador_casillero( Analizador* z, Casillero* cas ){
    ...
    if( z->pieza && CASILLERO_VALIDO( z->cas ) ) posicion_mueve_pieza( ZPOSICION(z), z->pieza, z->cas );
    return  STATUS_NORMAL;
}
...
int    analizador_jaquemate( Analizador* z, Tipopieza* tpieza ){
    ...
    for( i = 1; i <= z->pos.tjuego->colores ; i ++ ){
        ...
        if( posicion_en_jaque( ZPOSICION(z), tpieza, i ) ){
